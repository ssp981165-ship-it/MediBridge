...
export const LabPage = () => {
  ...
  const handleCardClick = (card) => {
    navigate(`/Labs/list/?category=${category}&id=${card._id}`);
  };
  if(id){
    return <Labb/>
  }
    ...
    <>
      ...
      <div className="cards-container">
        {Array.isArray(filteredCards) && filteredCards.length > 0 ? (
          ...
          <p>No Lab found for this category or search.</p>
        )}
      </div>
    </>
  );
...
//Note Doc card is lab card i used that just beacause doctor card is almost similar
